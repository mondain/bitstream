...
/**
 * This is a pseudo-protocol that allows the username and password for an http
 * datasource to be specified in the URL. Copied and modified from
 * URLDataSource. The syntax for an httpauth URL is derived from an http URL as
 * follows: http://myipcameraimage.com/cam with username "user" and password
 * "pass" would become httpauth:user:pass@//myipcameraimage.com/cam This is
 * simply a convenience data source. It allows things like FMJ studio to play IP
 * camera streams that are password protected, without having to prompt the
 * user.
 * 
 * @author Ken Larson
 * 
		...
		if (conn instanceof HttpURLConnection) { // TODO: this is probably why
			...
			if (user != null && !user.equals("")) {
				huc.setRequestProperty("Authorization", "Basic "
						+ StringUtils
								.byteArrayToBase64String((user + ":" + pass)
										.getBytes()));
			}
			huc.connect();
